...
public class Centrality2Excel {
	...
	public void writeDiseaseSpecificCentralityAverage(
			Map<String, Map<String, Map<String, Double>>> allAvgMap) throws IOException, JXLException, WriteException{
		if(!opened){
			open();
		}
		String newSheetname = "disease_average";
		System.out.println("Create sheet: " + newSheetname);
		...
		while(itr.hasNext()){
			entry = itr.next();
			avgMap = entry.getValue();
			Iterator tmpItr = avgMap.keySet().iterator();
			while(tmpItr.hasNext()){
				keyList.add((String) tmpItr.next());
			}
			break;		
		}
